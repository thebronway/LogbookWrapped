...
import { Share2, Bug, Check, Forward, HandCoins, TrendingUp, Plane } from 'lucide-react';
...
export const Page11_Export: React.FC<Props> = ({ onOpenExport, onOpenDonation, isExportMode }) => {
  const { dateFilter, rawFlights, setDateFilter, applyFilterAndCalculate, isSharedView, isDemo } = useLogbookStore();
  ...
  // Shared-view + demo viewers don't own the logbook, so the page-end CTA
  // block swaps to a "make your own" pitch.
  if ((isSharedView || isDemo) && !isExportMode) {
