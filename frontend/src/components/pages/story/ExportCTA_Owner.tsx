...
export const ExportCTA_Owner: React.FC<Props> = ({ onOpenExport, onOpenDonation }) => {
  ...
  const generateGrowthLink = () => {
    // For milestone and multi-year custom ranges, use the filter's own years
    if (dateFilter?.type === 'milestone' || dateFilter?.type === 'custom') {
      const y1 = dateFilter.start?.substring(0, 4);
      const y2 = dateFilter.end?.substring(0, 4);
      if (y1 && y2 && y1 !== y2) return { y1, y2 };
      return null;
    }

    // For all_time, use the actual min and max years in the data
    ...
    years.sort((a, b) => a - b);
    if (years.length >= 2) return { y1: years[0].toString(), y2: years[years.length - 1].toString() };
    return null;
  };
  ...
  // Show the growth button for multi-year queries only — hide for single-year or yoy
  const showGrowthButton =
    !!growthYears &&
    !['this_year', 'last_year', 'yoy'].includes(dateFilter?.type || '') &&
      ...
      <div className="relative flex flex-col shrink-0 w-full max-w-sm lg:max-w-full h-auto lg:h-full bg-slate-900 rounded-2xl lg:rounded-none shadow-2xl lg:shadow-none border border-slate-800 lg:border-none overflow-hidden mb-12 lg:mb-0">
        ...
        <div data-no-nav className={`p-6 pt-8 flex flex-col flex-1 justify-center gap-4 sm:px-12 sm:pb-12 ${!isReady ? 'pointer-events-none' : ''}`}>
          ...
          {showGrowthButton && (
            ...
            >
              <TrendingUp size={18} />
              View {growthYears!.y1} vs {growthYears!.y2} Growth
            </button>
          )}
